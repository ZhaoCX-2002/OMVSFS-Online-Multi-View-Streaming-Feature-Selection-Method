...
%data = randn(100,5); % dimensional normal data for 100 samples
%X = 1; Y = 2; S = [3,4]; % Test the independence of X = 1 and Y = 2 under the given S = [3, 4]
%[CI, r, p] = my_cond_indep_fisher_z(data,X,Y,S,100,0.05);
...
p = 2*(1 - normcdf(abs(W))); % 
%p = normcdf(W);%The p-value is one-sided, and the correct one should be the p-value for a two-sided test（2*(1 - normcdf(abs(W)))
r=abs(r);
%p = mynormcdf(W);
