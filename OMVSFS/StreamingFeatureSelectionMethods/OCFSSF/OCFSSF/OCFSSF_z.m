function  [pc,time,CSP]  = OCFSSF_z(data,target_feature,alpha,test)
...
cind=[];%条件独立集合
start=tic;
[n,numFeatures] = size(data);%得出数据集的实例数，特征数
...
for i = 1:numFeatures 
    ...
    stop=0;
    [CI,SS,dep]= my_cond_indep_fisher_z(data,i,target_feature,[],n,alpha);
    if CI==1 || isnan(dep) %与目标属性无条件独立的节点可能是配偶节点
            cind=[cind,i];
            ci=i;
            ccond{i}=[];
   ...
   if stop  %首先判断是不是PC节点，不是PC节点再判断是不是配偶节点 
        if ~isempty(selected_features)
            [CI,mcond]=compter_dep_2(selected_features,i,target_feature,3,0, alpha, test,data);
        end              
            ...
            if ~isempty(p2)
                 for j=1:p2
                     ...
                     if P==0%判断的节点不是新进入的节点
                        ...
                        if ~isempty(b)
                            [CI3,SS1,dep]=optimal_compter_dep_2(b,selected_features(j),target_feature,3, 0, alpha, test,data);%判断有没有因为新节点的进入，其他节点变得无关，如果无关则不考虑是PC节点，考虑该节点是否为配偶节点
                             if CI3==1 || isnan(dep)  %之前的节点变得独立，从CPC集合中移除，考虑该节点是不是配偶节点
    ...
    %                                 Unrelated_featurefeature=[Unrelated_featurefeature,selected_features(j)];
                                    ...
                                    for k=1:p2
                                        ...
                                        if KK==0
                                            ...
                                            SS=unique(SS);
                                            CI= my_cond_indep_fisher_z(data,selected_features(j),target_feature,SS,n,alpha);  %目标节点与该节点条件相关 
...
%                                                  if N==0%不是其中的节点  
                                                     ...
                                                     if ~isempty(nn)
                                                        for a=1:nn 
                                                            ...
                                                            [CI,S,dep]=optimal_compter_dep_21(m1,csp12(a),target_feature,3, 0, alpha, test,data,selected_features1(k));%判断每个节点与目标特征是否无关，如果无关，则去掉
                                                            if CI==1 || isnan(dep) 
    ...
    if ~isempty(pc) && ~isempty(cind)      
      ...
      if ci<GG
         ...
         for k1=1:plength1
             ...
             scind=unique(scind);
             CI5=my_cond_indep_fisher_z(data,cind(k1),target_feature,scind,n,alpha);
             if CI5==0%加入PC节点相关 可能为配偶节点
                 ...
                 for b=1:nn 
                    ...
                    [CI,s,dep]=optimal_compter_dep_21(m2,csp1(b),target_feature,3, 0, alpha, test,data,GG); %判断每个节点与目标特征是否独立，如果独立，则去掉
                    if CI==1 || isnan(dep) 
         ...
         for e=1:pp
            scind=[ccond{ci},pc(e)];
            scind=unique(scind);
            CI6=my_cond_indep_fisher_z(data,ci,target_feature,scind,n,alpha);
            if CI6==0%加入PC节点相关 可能为配偶节点
                ...
                for b=1:nn 
                    m2=[pc,csp];%CPC并与当前PC节点相关的配偶集合
                    m2=setdiff(m2,csp1(b),'stable');
                    [CI,s,dep]=optimal_compter_dep_21(m2,csp1(b),target_feature,3, 0, alpha, test,data,pc(e)); %判断每个节点与目标特征是否独立，如果独立，则去掉
                    if CI==1 || isnan(dep) 
